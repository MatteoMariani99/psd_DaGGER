...
def getHomographyFromPts():
    ...
    roadDesiredPts     = np.array([[ 140,307], [500,307], [500,400], [140,400]  ]).astype(np.float32)
    ...
    HomoMat = np.array([[   -0.46095,     -1.3316,      468.43],
...
def birdViewImg(blue_center, yellow_center, img):
    ...
    for pts in pts_blue:
        ...
        distance = abs(center_of_camera-pts)
    ...
    for pts in pts_yellow:
        ...
        distance = abs(pts-center_of_camera)
    ...
    idx_b = np.where((distance_to_cones_b[:,0] > 294) | (abs(distance_to_cones_b[:,1])>220))
...
def birdViewPts(blue_center: np.array, yellow_center: np.array, img):
    ...
    for pts in pts_bird_b:
        distance = abs(center_of_camera-pts)
        distance_to_cones_b.append(distance)
    
    
    
    for pts in pts_bird_y:
        distance = abs(pts-center_of_camera)
        distance_to_cones_y.append(distance)
    ...
    # 350 esco leggermente dall'immagine
    idx_b = np.where((abs(distance_to_cones_b[:,:,0]) > 650) | (abs(distance_to_cones_b[:,:,1])>900))
    idx_y = np.where((abs(distance_to_cones_y[:,:,0]) > 650) | (abs(distance_to_cones_y[:,:,1])>900))


    pts_yellow = np.delete(pts_bird_y, idx_y, axis=0)
    pts_blue = np.delete(pts_bird_b, idx_b, axis=0)
    ...
    # dist = lambda p : abs(p[0,0]-center_of_camera[0])+abs(p[0,1]-center_of_camera[1])
    # select = lambda v: v[np.array([dist(ve) for ve in v])<980]
    # pts_bird_b = select(pts_bird_b)
    # pts_bird_y = select(pts_bird_y)
    
    if pts_blue.shape[0]>0 and pts_yellow.shape[0]>0:
        ...
        pts_blue = np.concatenate((pts_blue, roadCenterL),axis=0)
        pts_yellow = np.concatenate((pts_yellow, roadCenterR),axis=0)
    
    if pts_blue.shape[0]==0:
        pts_blue = np.array([[[0, 480]]])
    if pts_yellow.shape[0]==0:
        pts_yellow = np.array([[[640,480]]])
    ...
    points = np.vstack((pts_blue, pts_yellow[::-1]))
    
    cv2.polylines(bird, [pts_blue], isClosed=False,color=(255), thickness=3, lineType=cv2.LINE_AA) 
    cv2.polylines(bird, [pts_yellow], isClosed=False,color=(255), thickness=3, lineType=cv2.LINE_AA) 
...
p.loadSDF("world/complete_track.sdf",globalScaling = 1)
...
while (1):
    ...
    for i in total_index:
        ...
        while r>0.5:
            ...
            blue_center, yellow_center = divide_centers(boxes_xywh, boxes_cls)

            #bird = birdViewImg(blue_center,yellow_center,img)
            bird = birdViewPts(blue_center, yellow_center, img)
            #print (bird.shape)
            #print(blue_center)
            #print(yellow_center)
