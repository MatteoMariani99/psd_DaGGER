...
class cameraMgr:
    ...
    def birdViewSpline(blue_center: np.array, yellow_center: np.array, img):
        ...
        if pts_bird_b.shape[0]<=1:
            ...
            if id_b.shape[0]==0:
                id_b = getLine(pts_bird_b).shape[0]-1
            else:
                id_b = id_b[-1]
                
       
        if pts_bird_y.shape[0]<=1:
            point = (getLine(pts_bird_b)[id_b].squeeze()[0]+10, 479) # 10 è l'offset per assicurarmi di essere all'interno
            
        elif pts_bird_y.shape[0]>1:
            ...
            if id_y.shape[0]==0:
                id_y = getLine(pts_bird_y).shape[0]-1
            else:
                id_y = id_y[-1]
            ...
            if pts_bird_b.shape[0]<=1:
                ...
                point = (int((getLine(pts_bird_y)[id_y].squeeze()[0]+getLine(pts_bird_b)[id_b].squeeze()[0])/2),479)
