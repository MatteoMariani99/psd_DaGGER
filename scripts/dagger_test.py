...
total_episode_step = 1999 # numero totale di step per episodio
...
if __name__ == "__main__":
    ...
    # istanza dell'ambiente
        env = ConesEnv(total_episode_step)
    else:
        env = RoadEnv(total_episode_step)
    ...
    for iteration in range(NUM_ITS):
        print("-----------------------------") 
        print("Algoritmo DAgger episodio: ",iteration) 
        print("")
        with tqdm(total=total_episode_step) as pbar:
            ...
            env.reset(True) # inizializzo l'ambiente e specifico i tracciati per il training
